...
const INDIAN_MEALS: Record<string, Meal[]> = {
  Breakfast: [
...
const GROCERY_ITEMS: GroceryItem[] = [
  { name: 'Basmati Rice (2kg)', category: 'Grains & Pulses', quantity: '1 pack', checked: false },
  { name: 'Toor Dal (1kg)', category: 'Grains & Pulses', quantity: '1 pack', checked: false },
  { name: 'Moong Dal (500g)', category: 'Grains & Pulses', quantity: '1 pack', checked: false },
  { name: 'Whole Wheat Atta (5kg)', category: 'Grains & Pulses', quantity: '1 pack', checked: false },
  { name: 'Poha (500g)', category: 'Grains & Pulses', quantity: '1 pack', checked: false },
  { name: 'Oats (1kg)', category: 'Grains & Pulses', quantity: '1 pack', checked: false },
  { name: 'Besan (500g)', category: 'Grains & Pulses', quantity: '1 pack', checked: false },
  { name: 'Paneer (400g)', category: 'Dairy & Eggs', quantity: '2 packs', checked: false },
  { name: 'Curd (400g)', category: 'Dairy & Eggs', quantity: '3 packs', checked: false },
  { name: 'Milk (1L)', category: 'Dairy & Eggs', quantity: '7 packets', checked: false },
  { name: 'Eggs (12 pack)', category: 'Dairy & Eggs', quantity: '1 tray', checked: false },
  { name: 'Ghee (500ml)', category: 'Dairy & Eggs', quantity: '1 bottle', checked: false },
  { name: 'Tomatoes (1kg)', category: 'Vegetables', quantity: '1 kg', checked: false },
  { name: 'Onions (1kg)', category: 'Vegetables', quantity: '1 kg', checked: false },
  { name: 'Potatoes (1kg)', category: 'Vegetables', quantity: '1 kg', checked: false },
  { name: 'Green Chillies (200g)', category: 'Vegetables', quantity: '1 pack', checked: false },
  { name: 'Coriander Leaves', category: 'Vegetables', quantity: '1 bunch', checked: false },
  { name: 'Spinach (500g)', category: 'Vegetables', quantity: '1 bunch', checked: false },
  { name: 'Carrots (500g)', category: 'Vegetables', quantity: '500g', checked: false },
  { name: 'Cumin Seeds', category: 'Spices', quantity: '100g', checked: false },
  { name: 'Turmeric Powder', category: 'Spices', quantity: '100g', checked: false },
  { name: 'Red Chilli Powder', category: 'Spices', quantity: '100g', checked: false },
  { name: 'Garam Masala', category: 'Spices', quantity: '50g', checked: false },
  { name: 'Mustard Seeds', category: 'Spices', quantity: '100g', checked: false },
  { name: 'Hing (Asafoetida)', category: 'Spices', quantity: '50g', checked: false },
  { name: 'Fruits (Banana, Apple)', category: 'Fruits', quantity: '1 dozen + 1kg', checked: false },
  { name: 'Peanuts (200g)', category: 'Snacks', quantity: '1 pack', checked: false },
  { name: 'Makhana (200g)', category: 'Snacks', quantity: '1 pack', checked: false },
];

function generateWeeklyPlan(): DayPlan[] {
  const days = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
  return days.map(day => {
    const pick = (pool: Meal[]): Meal => {
      const idx = Math.floor(Math.random() * pool.length);
      return { ...pool[idx], id: `${day}-${pool[idx].id}-${Math.random()}` };
    };
    return {
      day,
      meals: {
        Breakfast: pick(INDIAN_MEALS.Breakfast),
        Lunch: pick(INDIAN_MEALS.Lunch),
        Snack: pick(INDIAN_MEALS.Snack),
        Dinner: pick(INDIAN_MEALS.Dinner),
      },
    };
  });
}
...
export default function MealPlannerPage() {
  ...
  const swapMeal = (dayIdx: number, mealType: string) => {
    const pool = INDIAN_MEALS[mealType as keyof typeof INDIAN_MEALS];
    if (!pool) return;
        ...
        {swappingMeal && (
          ...
          >
            ...
            >
              ...
              <div className="space-y-2">
                {INDIAN_MEALS[swappingMeal.mealType as keyof typeof INDIAN_MEALS]?.map(meal => (
                  <button
                    key={meal.id}
