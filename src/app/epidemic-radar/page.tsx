...
export default function EpidemicRadarPage() {
  ...
  const [symptomTrends, setSymptomTrends] = useState(symptomTrendsDefault);

  const fetchRadarData = async (disease?: string) => {
    try {
      const params = new URLSearchParams();
      if (disease && disease !== 'all') params.set('disease', disease);
      const res = await fetch(`/api/epidemic-radar?${params}`);
      if (!res.ok) throw new Error('Failed');
      const data = await res.json();
      if (data.outbreakData) setOutbreakData(data.outbreakData);
      if (data.heatmapPoints) setHeatmapPoints(data.heatmapPoints);
      if (data.symptomTrends) setSymptomTrends(data.symptomTrends);
  ...
  useEffect(() => {
    fetchRadarData(selectedDisease);
  }, [selectedDisease]);
