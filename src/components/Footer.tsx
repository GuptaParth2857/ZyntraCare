        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5 }}>
            <Link href="/" className="flex items-center gap-2 mb-3">
              <div className="w-10 h-10 rounded-full overflow-hidden border-2 border-teal-400/50 shadow-[0_0_10px_rgba(20,184,166,0.3)]">
                <Image src="/images/publiczyntracare-logo.png" alt="ZyntraCare Logo" width={40} height={40} className="object-cover" />
              </div>
              <span className="font-black text-xl text-white font-outfit">Zyntra<span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-400 to-gold-400">Care</span></span>
            </Link>
            <p className="text-gray-400 text-sm mb-3">India's most trusted healthcare platform.</p>
            <div className="flex gap-3">
              {[FiFacebook, FiTwitter, FiInstagram, FiLinkedin].map((Icon, idx) => (
                <Icon key={idx} size={18} className="text-gray-400 hover:text-teal-400 cursor-pointer transition" />
              ))}
            </div>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.1 }}>
            <h4 className="font-bold text-sm mb-3 text-teal-400">Explore</h4>
            <ul className="space-y-1.5">
              {[
                ...
                <li key={link.href}>
                  <Link href={link.href} className="text-gray-400 hover:text-teal-400 transition text-sm">{link.label}</Link>
                </li>
              ))}
            </ul>
          </motion.div>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: 0.3 }}>
            ...
            <ul className="space-y-2 text-sm">
              ...
              <li className="flex items-center gap-2 text-gray-400">
                ...
                <a href="mailto:contact@zyntracare.com" className="hover:text-teal-400 text-xs">Email Us</a>
              </li>
              <li className="flex items-start gap-2 text-gray-400">
                <FiMapPin className="text-teal-400 shrink-0 text-sm mt-0.5" />
            ...
            <div className="mt-4 pt-3 border-t border-gray-800">
              ...
              <div className="flex flex-wrap gap-2">
                <Link href="/hospital-dashboard" className="text-xs text-gray-400 hover:text-teal-400">Hospital</Link>
                <Link href="/doctors/register" className="text-xs text-gray-400 hover:text-teal-400">Doctor</Link>
                <Link href="/corporate-wellness" className="text-xs text-gray-400 hover:text-teal-400">Corporate</Link>
              </div>
            </div>
